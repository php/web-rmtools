...
set LOG_FILE=c:\php-sdk\logs\task-nts-%cur_date%.log
set RMTOOLS_BASE_DIR=c:\php-sdk\rmtools-client

IF EXIST c:\php-sdk\locks\snaps_nts.lock (
ECHO Snapshot script is already running.
GOTO EXIT_LOCKED
)

ECHO running > c:\php-sdk\locks\snaps_nts.lock 
...
del c:\php-sdk\locks\snaps_nts.lock >> %LOG_FILE% 2<&1
